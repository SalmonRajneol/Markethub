...
if ($action == 'a') {
	...
	$id = $_REQUEST["id"];
	 	$query = "select * from stuTab where stuID = $id";
...
<form action="studentJ_action4.php" method="get" name="form1" >
  <table width="59%" border="0" cellspacing="0" cellpadding="5">
    ...
    <tr> 
      ...
      <td><label>
        <select name="stuMajorID" id="select">
    

<?php 
	
$query = "SELECT * FROM majorTab order by majorName";
	$result = mysqli_query($conn, $query) or die(mysqli_error());
	while ($row = mysqli_fetch_array($result)) {
		$majorID = $row['majorID'];
		$majorName = $row['majorName'];
		if ($majorID == $stuMajorID) {
			$selected = " Selected ";
		} else {
			$selected = NULL;
		}
		$option = '<option value="' . $majorID . '"' .  $selected . '>' . $majorName . '</option>';
		echo $option . "\n";
	} // end while
?>
...
<p><a href="studentJ_list4.php">Return </a></p>
</body>
</html>
